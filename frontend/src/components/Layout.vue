...
export default {
  ...
  setup() {
    ...
    const canAccessPhase = (phase) => {
      ...
      switch (phase) {
        case 2:
          // Phase 2: Check if Phase 1 has jobs selected
          const selectedJobs = localStorage.getItem('selectedJobs')
          return selectedJobs && JSON.parse(selectedJobs).length > 0
        case 3:
          // Phase 3: Check if Phase 2 has generated resumes
          const generatedResumes = localStorage.getItem('generatedResumes')
          return generatedResumes && Object.keys(JSON.parse(generatedResumes)).length > 0
        case 4:
