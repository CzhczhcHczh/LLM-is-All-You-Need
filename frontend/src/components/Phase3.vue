  ...
  <div class="phase3-container">
    ...
    <div class="navigation-bar">
      <div class="nav-steps">
        <el-steps :active="2" finish-status="success" align-center>
          <el-step title="职位搜索" @click="navigateToPhase(1)"></el-step>
          <el-step title="简历制作" @click="navigateToPhase(2)"></el-step>
          <el-step title="HR模拟" @click="navigateToPhase(3)"></el-step>
          <el-step title="面试安排" @click="navigateToPhase(4)"></el-step>
        </el-steps>
      </div>
      <div class="nav-buttons">
        <el-button @click="navigateToPhase(2)" type="info" plain>
          <el-icon><ArrowLeft /></el-icon>
          返回简历制作
        </el-button>
        <el-button @click="navigateToPhase(4)" type="primary" :disabled="!canNavigateToPhase4">
          进入面试安排
    ...
    <el-card v-if="selectedFeedback" class="feedback-card">
      ...
      <div class="feedback-actions">
        <div class="action-group optimization-group">
          <h5>简历优化 (可选)</h5>
          <el-button @click="optimizeResume" type="warning" size="large">
            <el-icon><Edit /></el-icon>
            根据反馈优化简历
          </el-button>
          
          <el-button @click="showOptimizedResume" type="primary" v-if="hasOptimizedResume" size="large">
            <el-icon><View /></el-icon>
            查看优化后简历
          </el-button>
          
          <el-button @click="downloadOptimizedResume" type="success" v-if="hasOptimizedResume" size="large">
    ...
    <!-- 新增：优化后简历展示对话框 -->
    <el-dialog
      v-model="optimizedResumeDialogVisible"
      title="优化后简历预览"
      width="90%"
      :show-close="true"
      class="resume-dialog"
    >
      <div class="dialog-actions" style="margin-bottom: 20px;">
        <el-button @click="downloadOptimizedResume" type="success" size="large">
          <el-icon><Download /></el-icon>
          下载Word简历
        </el-button>
      </div>
      ...
      <ResumeDisplay 
        v-if="optimizedResumeData" 
        :resume-data="optimizedResumeData"
        :job-posting="currentJobPosting"
      />
    </el-dialog>
...
export default {
  ...
  setup() {
    ...
    const optimizeResume = async () => {
      ...
      try {
        ...
        if (response.success !== false) {
          ...
          // 标记为已优化
          resume.optimizedAt = new Date().toISOString()
          resume.originalData = resume.originalData || { ...resume.data } // 保存原始数据
          
          // 保存优化后的数据到组件状态
          optimizedResumeData.value = resume.data
          currentJobPosting.value = resume.jobPosting
          ...
          ElMessage.success('简历已根据HR反馈进行优化！点击"查看优化后简历"预览效果')
    ...
    const showOptimizedResume = () => {
      ...
      optimizedResumeData.value = resume.data
      currentJobPosting.value = resume.jobPosting
      optimizedResumeDialogVisible.value = true
    }
    ...
    const downloadOptimizedResume = async () => {
      ...
      try {
        ...
        // 使用Phase2中相同的下载逻辑
        const response = await apiService.downloadResumeWord({
          resume: resume.data,
          job: resume.jobPosting
        })
        ...
        if (response.success) {
          ...
          // 生成文件名
          const candidateName = resume.data.personal_info?.name || '候选人'
          const jobTitle = resume.jobPosting?.job_title || '职位'
          const timestamp = new Date().toISOString().slice(0, 10)
          a.download = `${candidateName}_${jobTitle}_优化简历_${timestamp}.docx`
          ...
          ElMessage.error('Word文档生成失败：' + (response.message || '未知错误'))
        }
      } catch (error) {
    ...
    return {
      ...
      viewFeedback,
      optimizeResume,
      showOptimizedResume,         // 新增：显示优化后简历
      downloadOptimizedResume,     // 新增：下载优化后简历
      calculateTotalScore,
      getDimensionName,
      getHRPersonaName,
      getScoreColor,
...
.nav-steps .el-steps {
  --el-steps-simple-border-color: rgba(255, 255, 255, 0.3);
  --el-steps-simple-bg-color: rgba(255, 255, 255, 0.1);
}
