...
export default {
  ...
  setup() {
    ...
    const proceedToPhase3 = () => {
      ...
      // 提取并保存个人信息供Phase4使用
      const firstResume = Object.values(generatedResumes.value)[0]
      if (firstResume && firstResume.personal_info) {
        localStorage.setItem('userProfile', JSON.stringify(firstResume.personal_info))
        console.log('保存个人信息到userProfile:', firstResume.personal_info)
      }
